...
# Download the GitHub repository ZIP from the main branch
...
# Find the extracted folder (GitHub zips the repo with the branch name)
...
# Check folder structure (debugging output)
Write-Host "Listing folder structure inside '$extractPath\flux'..."
Get-ChildItem -Recurse -Path "$extractPath\flux" | ForEach-Object { Write-Host $_.FullName }
...
# Copy install scripts to nodejs directory (check path based on structure)
Write-Host "Copying install scripts to $nodejsPath..."
$installPath = "$extractPath\flux\install"
